...
def mutate(repo: pathlib.Path, which: str) -> None:
    if which == "t05-old-slice":
        ...
        print(f"mutated {backend}: budgeted Winpty read ignores its budget")
    elif which == "a04-s1-drained-when-full":
        session = repo / "smartcli_core" / "session.py"
        _replace_once(
            session,
        ...
        print(f"mutated {session}: a full turn claims the stream is drained")
        ...
        print(f"mutated {session}: a legacy backend is read unbounded and sliced")
    else:
