...
class PtySession:
    ...
    def _parser_incomplete(self) -> bool | None:
        """Is the transport-level filter holding an unfinished control sequence?

        Read from the reviewed streaming filter's own state. It reports True/False
        for that filter only; it cannot prove pyte's own decoder has nothing
        buffered, so a False here means "this filter is at ground", not "the
        parser buffer is empty".
        """
        state = getattr(self.model.stream, "_state", None)
        ground = getattr(type(self.model.stream), "_GROUND", None)
        if state is None or ground is None:
            return None
        return bool(state != ground)
