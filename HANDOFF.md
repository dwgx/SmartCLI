...
8. **[DONE 2026-08-09] Docs site + contributor onramp:** `CONTRIBUTING.md` + `SECURITY.md` DONE (v0.1.4); Codecov badge live; **Read the Docs is LIVE** at https://smartcli.readthedocs.io/ (mkdocs, via `.readthedocs.yaml` + `tools/build_docs.py` which rewrites repo-relative links to absolute GitHub URLs). **The docs build is now a blocking CI gate** (`mkdocs build --strict` in lint.yml, added 2026-08-09) — it previously ran nowhere and the site could rot silently. **Optional, deliberately not done:** `TestPyPI` (rehearsal channel; everything it would verify already happens in CI's package job and pre-tag checks — see PACKAGING-NOTES for the reasoning), `conda-forge` (recipe is finished and verified; submission is a one-time PR whenever conda users are wanted), `Homebrew` (verdict: do NOT publish — see the formula header).
