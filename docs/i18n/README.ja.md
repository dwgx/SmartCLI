...
ローカル開発は Windows 11、Python 3.14.6、`pyte` + `pywinpty` / ConPTY 上で行っており、CI は Windows + Linux + macOS のマトリクスで検証しています。**知覚が正しいと分かる理由。** スクリーンモデルは実際の端末と一致してこそ意味があるため、そこは主張ではなく測定しています。同一のバイト列を実際の **tmux** ペインと本モデルの両方に流し、2 つのセルグリッドをセル単位で差分します。これを行うスイートが 3 つ —— 35 件の厳選ケース、**tmux と GNU screen の双方が一致したときだけ**その挙動を正解とみなす 3 者照合、そしてランダムな VT シーケンスに対する生成的ファズ。この取り組みで **12 件のエミュレーション不具合**を発見・修正しました（`pyte` は 1049/1047/47 をいずれも実装しておらず、全画面プログラムの出力がメイン画面の上に描かれ、終了後も復元されないという代替スクリーンバッファの問題を含む）。範囲と残る境界例は [`LIMITATIONS.md`](../../skills/drive-tui/references/LIMITATIONS.md) を参照 とラベル付けされています。
